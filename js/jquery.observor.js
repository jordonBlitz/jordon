 ...
 * @version     $Id: jquery.observor.js 52 2014-08-07 02:36:13Z bizlogic $
 *
 * @category    JavaScript
...
$(document).ready(function() {	
	$('.adminActionTrigger').live({
		mouseenter: function (event) {
			$(event.currentTarget).find('.adminAction').show();
	    },
	    mouseleave: function (event) {
	    	$(event.currentTarget).find('.adminAction').hide();
	    }
	});
	...
	$('#aboutJordon').click(function(event) {	
		var thisYear = new Date().getFullYear();
		
		var text = '<strong>Jordon</strong> 1.0<br>Project Management & Issue Tracking<br><br>&copy; Copyright 2014 - '+ thisYear +' BizLogic<br>All Rights Reserved.<br><a href="http://jordonblitz.com" target="_blank">http://jordonblitz.com</a>';
	...
	$('#linkLogout').click(function(event) {
		if( typeof CURRENT_USER['external'] !== 'undefined' ) {
			var network = CURRENT_USER['external']['network'];
	...
	$('.changeTheme').live('click', function(event) {
		...
		var themeName	= $(this).data('displayname');
		...
		if( theme.length > 0 ) {
			...
			// change link name
			$('#linkThemeCurrent').data('name', theme).data('displayname', themeName).html( themeName );
