 ...
 * @version     $Id: LoginController.php 56 2014-06-14 16:21:49Z hire@bizlogicdev.com $
 *
...
class LoginController extends Zend_Controller_Action
{
    ...
    public function ajaxAction()
    {
		...
		if( !empty( $_POST ) ) {	
			...
			switch( $method ) {
				case 'userLogin':					
					$data		= $this->_CloneUI_User->login( $_POST['username'], $_POST['password'] );						
					$json		= array();
					$json['data']	= $data;
    ...
    */    
    ...
    {    	    	
    	...
    	if( strlen( trim( $username ) ) AND strlen( trim( $password ) ) ) {    		    		
    		$loggedIn = $this->_CloneUI_User->login( $username, $password );
    ...
    public function twitterAction()
    {
    	...
    	if( strlen( @$_COOKIE['twitter_access_token'] ) ) {
    		...
    		if( strlen( $response->screen_name ) ) {
    			...
    			$this->_CloneUI_User->loginExternal( $response );
