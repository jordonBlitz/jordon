...
class CloneUI_User
{
	...
	public function __construct()
	{
		...
		$this->columnNames				= fetchColumnNames( $this->tableName );
		$this->_CloneUI_Language		= new CloneUI_Language;
	...
	*/
		...
		*/
		...
		while ( $aRow = mysql_fetch_array( $rResult ) ) {
			...
			for ( $i = 0; $i < count( $aColumns ); $i++ ) {
				switch( $aColumns[$i] ) {
					case 'site_language':
						$row[] = $this->_CloneUI_Language->fetchFriendlyNameById( $aRow[ $aColumns[$i] ] );
						break;
	...
	public function banUserById( $userId )
	{
		...
		return $this->_CloneUI_Usergroup->setUsersUsergroupById( $userId, 0 );
	}
	...
	public function deleteUserById( $userId )
	{
		...
		$media = $this->_CloneUI_User_Media->fetchAllMediaIdsByUserId( $userId );
		
		if( !empty( $media ) ) {
			$this->_CloneUI_User_Media->deleteMultipleMediaById( $media );			
		}
	...
	public function fetchAllUsers( $columns = '*', $offset = 0, $limit = 20, $orderBy = 'username', $sortOrder = 'ASC' )
	{
		...
		$sql    = "SELECT ".mysql_real_escape_string( $columns )." FROM `".DB_TABLE_PREFIX."user` ";
    ...
    */
    ...
    {
    	// this is used to track recently browsed projects
    ...
    {
    	return $this->_CloneUI_Site_Permission->fetchSitePermissionsByUserId( $userId );    	
    }
    
    public function fetchSitePermissionsExternal()
    {
    	return $this->_CloneUI_Site_Permission->fetchSitePermissionsExternal();
    }
    ...
    public function changeUserLangByLangId( $langId )
    {
    	...
    	if( !$this->_CloneUI_Language->languageExistsById( $langId ) ) {
    		return false;	
    	}
